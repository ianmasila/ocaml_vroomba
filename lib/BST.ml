...
*)
  ...
  let mk_node e = { value = e; parent = ref None; left = ref None; right = ref None }

  let mk_tree _ = { root = ref None; size = ref 0 }

  let map_option o f z =
    match o with
    | None -> z
    | Some n -> f n
  ...
  let delete_node t z =
    ...
    then transplant t z (left z)
      ...
      if parent y <> None && z != get_exn @@ parent y
      then (
  ...
  let left_rotate t x =
    ...
    | Some y ->
      ...
      x.right := left y;
      if left y <> None then (get_exn @@ left y).parent := Some x;
...
let print_kv_tree = print_tree (fun (k, v) -> Printf.sprintf "(%d, %s)" k v) 12
