...
let polygon_of_int_pairs l =
  List.map (fun (x, y) -> Point (float_of_int x, float_of_int y)) l
...
let draw_polygon ?(color = Graphics.black) p =
  let open Graphics in
  let open GraphicUtil in
  set_color color;
  List.map
    (function
      | Point (x, y) -> (int_of_float x + fst origin, int_of_float y + snd origin))
    p
  |> Array.of_list
  |> draw_poly;
  set_color black
...
module TestPolygons = struct
  let triangle = [(-50, 50); (200, 0); (200, 200)] |> polygon_of_int_pairs

  let square =
    [(100, -100); (100, 100); (-100, 100); (-100, -100)] |> polygon_of_int_pairs
  ...
  let simpleNonConvexPoly =
    [(0, 0); (200, 0); (200, 200); (100, 50)] |> polygon_of_int_pairs

  let nonConvexPoly5 =
    [(0, 0); (0, 200); (200, 200); (-100, 300)]
    |> polygon_of_int_pairs
    |> shift_polygon (-50., -100.)

  let bunnyEars =
    [(0, 0); (400, 0); (300, 200); (200, 100); (100, 200)]
    |> polygon_of_int_pairs
    |> shift_polygon (-100., -50.)

  let lShapedPolygon =
    [(0, 0); (200, 0); (200, 100); (100, 100); (100, 300); (0, 300)]
    |> polygon_of_int_pairs
    |> shift_polygon (-150., -150.)

  let kittyPolygon =
    [(0, 0); (500, 0); (500, 200); (400, 100); (100, 100); (0, 200)]
    |> polygon_of_int_pairs
    |> shift_polygon (-250., -150.)

  let simpleStarPolygon =
    [
      (290, 0);
      (100, 100);
      (0, 290);
  ...
  let sand4 =
    [(0, 0); (200, 0); (200, 100); (170, 100); (150, 40); (130, 100); (0, 100)]
    |> polygon_of_int_pairs
    |> shift_polygon (-30., -30.)

  let tHorror =
    [(100, 300); (200, 100); (300, 300); (200, 300); (200, 400)]
    |> polygon_of_int_pairs
  ...
  let shurikenPolygon =
    [(390, 0); (200, 50); (0, 290); (50, 150); (-200, -100); (0, 0)]
    |> polygon_of_int_pairs
    |> shift_polygon (-80., -70.)
end
...
let rotate_polygon pol p0 angle =
  ...
  |> List.map (function Polar (r, phi) -> Polar (r, phi +. angle))
...
let is_convex pol =
  all_triples pol |> List.for_all (fun (p1, p2, p3) -> direction p1 p2 p3 <= 0)
...
let polygons_touch_or_intersect pol1 pol2 =
  ...
  List.exists (fun e1 -> List.exists (fun e2 -> segments_intersect e1 e2) es2) es1
...
let draw_ray ?(color = Graphics.black) r =
  let p, phi = r in
...
let ray_segment_intersection ray seg =
  ...
  (* Ray's direction *)
  let r = Point (cos phi, sin phi) in
  if cross_product s r =~= 0.
  then
    if cross_product (p -- q) r =~= 0.
    then
      if point_on_ray ray p then Some p else if point_on_ray ray p' then Some p' else None
    else None
  else (
    (* Point on segment *)
...
let choose_ray_angle pol p =
  ...
  let candidate_angles =
    iota (n + 1) |> List.map (fun i -> float_of_int i *. pi /. float_of_int n)
  in
  let phi =
    List.find
      (fun c ->
        List.for_all (fun a -> not (a =~= c)) edge_angles
        && List.for_all (fun a -> not (a =~= c)) vertex_angles)
      candidate_angles
  in
  phi
...
let point_within_polygon pol p =
  ...
  let es = edges pol in
  if List.mem p pol || List.exists (fun e -> point_on_segment e p) es
  then true
  else (
    let n =
      edges pol
